...
**Особенности:**
- Маска применяется автоматически сразу после сохранения (без перезапуска сервиса)
- Поддержка отдельной маски для каждой камеры
- Маски хранятся в `/opt/lpr/data/cameras.json`

**Режимы проверки маски** (настройка `mask_mode`):
| Режим | Описание |
|-------|----------|
| bottom | Нижний центр bbox (рекомендуется) |
| center | Геометрический центр bbox |
| all_corners | Все 4 угла bbox должны быть в маске |
